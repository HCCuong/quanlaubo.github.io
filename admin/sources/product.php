            ...
            if(isset($config['product'][$type]['schema']) && $config['product'][$type]['schema'] == true)
            {
                ...
                if($buildSchema) {
                    foreach($config['website']['seo'] as $k => $v) {
                        ...
                        //Tiến hành build schema product
                        $dataSchema['schema'.$k]=$func->buildSchemaProduct($id_insert,$data['name'.$k],$configBase.UPLOAD_PRODUCT_L.$photoUpdate['photo'],$dataSeo['description'.$k],$data['code'],$pro_list['ten'],$setting['name'.$k],$url_pro,$data['regular_price']);
                    }
                }else{
                    $dataSchema = (isset($_POST['dataSchema'])) ? $_POST['dataSchema'] : null;
                    if($dataSchema)
